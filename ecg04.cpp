...
void setup_matrices(){
	...
	vec4 pos = rotate_y(angle)*vec4{20, 8, 20, 1};
	Uniform{"View"} = View = lookAt(toVec3(pos), {0, 4, 0}, {0, 1, 0});
}
	
void setup_light(float scale){
	Uniform{"light_position"} = vec4{ 0.0, 8.0, 10.0, 1.0 };
	Uniform{"Ia"} = scale*vec3{ 0.2, 0.2, 0.2};
	Uniform{"Id"} = scale*vec3{ 1.0, 1.0, 1.0};
	Uniform{"Is"} = scale*vec3{ 1.0, 1.0, 1.0};
}
